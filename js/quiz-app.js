...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTopics() {
        ...
        for (const [topic, count] of entries) {
            ...
            `;
            div.addEventListener('change', () => toggleTopic(topic));
    ...
    const startQuiz = async () => {
        ...
        try {
            ...
            for (const topic of selectedTopics) {
                const filename = topic.toLowerCase().replace(/ /g, '-');
    ...
    const startRealExam = async () => {
        if (selectedTopics.length === 0) {
            alert('Por favor, selecione pelo menos um tópico para o exame.');
            return;
        }
        ...
        try {
            // Calculate total questions based on topicConfig
            const totalQuestions = selectedTopics.reduce((total, topic) => total + topicConfig[topic], 0);
            ...
            for (const topic of selectedTopics) {
                ...
                const selected = shuffled.slice(0, topicConfig[topic]);
            ...
            // Take only the first 90 questions for the real exam
    ...
    const submitA = (isAutoSubmit = false) => {
        ...
        if (isAutoSubmit) {
            ...
            alert(`Exame concluído!\n\n` +
                  `Pontuação: ${score}/${courseConfig?.maxScore || 900}\n` +
    ...
    const showFeedback = (q, selected) => {
        ...
        if (resultMsg) {
            resultMsg.textContent = isCorrect ? '✅ Correto!' : '❌ Incorreto';
